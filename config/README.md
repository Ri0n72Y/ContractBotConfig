...
OPENCONTRACTS_UPLOAD_WORKER_KEY=<contracts-history-bound WorkerKey>
```

The current OpenContracts deployment exposes `contracts-history` and `contract-templates` as the two public runtime corpuses. `config/opencontracts.env.example` carries those two slugs as defaults while leaving host-specific URLs, CA paths and secrets blank.
...
Only two OpenContracts Corpus identities are configured by the Skill Pack: history (`contracts-history`) and templates (`contract-templates`). Session experience/learning material stays outside OpenContracts and is handled through local notes plus manual Skill updates.

## Formal ingestion

Formal contract ingestion uses a Corpus-bound WorkerKey bound to `contracts-history`. OpenContracts' `mint_worker_token` command prints the plaintext token once. Copy it to `OPENCONTRACTS_UPLOAD_WORKER_KEY` on the Agent/Harness host.
