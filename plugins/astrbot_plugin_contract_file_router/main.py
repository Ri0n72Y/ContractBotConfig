...
class ContractFileRouter(Star):
    ...
    async def _component_path(self, component: Any) -> str | None:
        ...
        if callable(converter):
            try:
                ...
                logger.warning("Contract file conversion failed: %s", exc)
        return None
    ...
    def _build_task_context(
        ...
        if (
            ...
            or (record.get("duplicate_confirmation_id") and record.get("duplicate_confirmed_at"))
        ):
    ...
    async def intake(
        ...
        if (
            ...
            yield event.plain_result(RUNNING_PROMPT)
            return
        if state in {"upload_started", "task_started", "duplicate_confirmed"} and record.get("dispatch_task_id"):
    ...
    async def clear_pending_after_result(
        ...
        **_kwargs: Any,
        ...
        record = self.pending.get(session)
        preserve = (
            event.get_extra("contract_preserve_pending_reason")
            == "duplicate_confirmation_required"
        )
        if preserve and isinstance(record, dict):
            record["state"] = "awaiting_duplicate_confirmation"
