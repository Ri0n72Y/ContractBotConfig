...
REQUIRED_BUILDER_TOOLS = {"list_documents", "get_document_text", "docassemble_generate_document", "publish_contract_download"}
...
class ContractGenerationFlow(Star):
    def __init__(self, context: Context, config: AstrBotConfig | None = None) -> None:
        ...
        self.progress_text = str(config.get("generation_progress_text", "正在读取合同库中的参考合同并生成 DOCX，完成后会返回下载链接。")).strip()

    async def initialize(self) -> None:
        logger.info("Contract generation flow 0.2.0 initialized.")

    @staticmethod
    def _resolve_provider_request(hook_args: tuple[Any, ...], hook_kwargs: dict[str, Any]) -> ProviderRequest | None:
        candidate = hook_kwargs.get("req") or hook_kwargs.get("request")
        if isinstance(candidate, ProviderRequest):
            return candidate
        for value in hook_args:
            if isinstance(value, ProviderRequest) or (hasattr(value, "func_tool") and hasattr(value, "prompt")):
                return value
        return None

    @staticmethod
    def _resolve_tool_args(hook_args: tuple[Any, ...], hook_kwargs: dict[str, Any]) -> tuple[Any | None, dict[str, Any] | None]:
    ...
    async def _send_progress_once(self, event: AstrMessageEvent) -> None:
        if not self.progress_enabled or not self.progress_text or event.get_extra("contract_generation_progress_sent", False):
            return
        try:
    ...
    async def prepare_builder_request(self, event: AstrMessageEvent, *hook_args: Any, **hook_kwargs: Any) -> None:
        ...
        if isinstance(tools, list):
            ...
            if missing:
                tool_set.tools = []
                self._append_temp_instruction(req, "<contract_generation_tool_guard>Builder 核心运行工具绑定不完整。missing_tools=" + json.dumps(missing, ensure_ascii=False) + "。直接返回 [CONTRACT_DOCASSEMBLE:BLOCKED]，reason=builder_tool_binding_incomplete。</contract_generation_tool_guard>")
                logger.error("Contract generation flow: Builder core tools incomplete: %s", missing)
                return
        self._append_temp_instruction(req, "<contract_generation_execution_policy>这是已经进入执行阶段的合同草稿生成任务，不存在额外确认门。用户要求生成、起草、制作或按当前方案生成即视为执行授权。优先从本轮合同库参考正文补齐可复用信息；用户未提供且合同库也没有的信息默认保留【待填写】占位符并继续，除非用户明确要求缺失字段必须停止。不要为了普通草稿字段再次向用户提问。</contract_generation_execution_policy>")

    @filter.on_using_llm_tool(priority=1050)
    async def mark_generation_execution(self, event: AstrMessageEvent, *hook_args: Any, **hook_kwargs: Any) -> None:
        tool, tool_args = self._resolve_tool_args(hook_args, hook_kwargs)
        if tool is None or tool_args is None or str(getattr(tool, "name", "")) != GENERATION_TOOL:
            return
        event.set_extra("contract_docassemble_generation_task", True)
        event.set_extra("contract_generation_confirmation_approved", True)
        event.set_extra("contract_generation_download_publication_verified", False)
        event.set_extra("contract_generation_original_handoff_input", tool_args.get("input"))
        tool_args["background_task"] = False
        await self._send_progress_once(event)

    @filter.on_using_llm_tool(priority=900)
    async def restore_generation_handoff_after_legacy_guards(self, event: AstrMessageEvent, *hook_args: Any, **hook_kwargs: Any) -> None:
        tool, tool_args = self._resolve_tool_args(hook_args, hook_kwargs)
        if tool is None or tool_args is None or str(getattr(tool, "name", "")) != GENERATION_TOOL:
            return
        ...
        if original is not None:
            ...
            tool_args["background_task"] = False
            logger.warning("Contract generation flow: restored Builder handoff rewritten by legacy duplicate guard.")
