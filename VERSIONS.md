...
File Router 0.5.9 不再维护持久化 current-file 指针；`pending` 只表示未完成的文件任务。普通任务完成以及“结束/取消”会清除相关任务状态，但业务流程不物理删除已接收暂存文件，也不提供用户侧删除入口。`contract-conversation-control` 1.16 已同步这一生命周期语义；BLOCKED 恢复只保留未完成任务所需的文件引用和业务状态。临时文件以 MD5 作为身份和短时重复判定依据，SHA-256 继续保留给下游完整性/上传链。长期未使用文件的月度清理由独立维护任务后续实现。
