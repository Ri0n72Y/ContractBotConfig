...
- 正式客户生成不再以“调用过 list_documents/get_document_text”作为通过条件：Generation Flow 0.1.2 使用 `on_llm_tool_respond` 核验真实结果；`list_documents` 必须返回非空文档列表，`get_document_text` 必须对本轮列表中的 slug 从 offset 0 返回 `total_chars > 0` 且正文非空；
- Docassemble Gateway 0.1.3 只接受 `contract_generation_reference_list_verified=true` 与 `contract_generation_reference_text_verified=true` 的正式生成请求；OpenContracts error、空列表、空正文或未关联 slug 均不能解锁 Docassemble；
...
- Docassemble Gateway 0.1.3 使用 `http://docassemble`、API Key 和 allowlist interview 调用官方 session/file API，并允许 Builder 在生成完成后调用受控下载交付工具；
- Docassemble Gateway 本地 `output_dir` 现在默认只保留 Gateway 自身生成的 DOCX 86400 秒，每 300 秒安全扫描一次；清理器只删除直属、符合 `12位十六进制前缀_*.docx` 规则的过期普通文件，不递归处理未知内容；
