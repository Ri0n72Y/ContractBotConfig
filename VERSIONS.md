...
`contract-direct-analysis` 1.15 默认把企业微信快速分析压缩为最高优先级的 4–6 个风险（最多 8 个），每项直接给风险、原文位置和修改建议，不再机械输出完整法律备忘录。`contract-conversation-control` 1.16 与 File Router 0.5.9 统一文件保留语义：任务完成后继续保留当前文件；“结束”只结束文件会话，“取消”只取消当前操作，明确“删除文件”才物理删除；直接上传下一份文件会切换当前文件，但上一份暂存文件仍保留等待后续维护清理。
...
- contract_master_orchestrator: 1.27（generation policy protocol 2；默认简洁合同分析；不使用 Shell 扫描附件或 Skill 文件）
